...
Handover report (8 September 2026, sections 20 and 31): a malformed model
response escaped the code-change stage and terminated the worker process.

Contract pinned here: only the code-change stage is failure-isolated
(it already fails missions safely with sanitized error state). Every other
stage keeps its existing propagation semantics so systemic errors surface.
"""

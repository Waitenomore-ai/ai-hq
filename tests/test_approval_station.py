...
def build_client():
    ...
    # Development environment: the origin check is production-only and the
    # production DripVid MCP socket path validation is not portable to hosts
    # without POSIX absolute paths. CSRF/session protections are exercised
    # here; origin binding is enforced by the same helpers in production.
    settings = Settings(
...
def test_expired_approval_can_be_decided_but_never_authorizes():
    """Expiry is enforced at consumption time (pinned contract).

    Deciding an expired request must not fail, but is_approved() must stay
    False, so an expired approval can never authorize publication.
    """
...
def test_approval_form_requires_csrf_token():
    ...
    assert (
        ApprovalService(factory).get_request(approval.id).state.value
        == "pending"
    )
